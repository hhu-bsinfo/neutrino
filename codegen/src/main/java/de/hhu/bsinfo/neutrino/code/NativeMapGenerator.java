...
public class NativeMapGenerator {
    ...
    public static String generate(String structName, List<StructMember> members) {
        ...
        String memberInfos = String.format("ReflectionUtility::MemberInfo %s_member_infos[] = {\n%s\n};", structName, memberInfosBody);

        String structInfo = String.format("ReflectionUtility::StructInfo %s_struct_info {\n\tsizeof(%s),\n\tsizeof(%s_member_infos) / sizeof(ReflectionUtility::MemberInfo),\n\t%s_member_infos\n};", structName, structName, structName, structName);
