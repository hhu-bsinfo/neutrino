        ...
        try (var socket = new Socket(serverAddress.getAddress(), serverAddress.getPort())) {
            ...
            RSocketFactory.connect()
                    ...
                    .flatMapMany(service -> {
                        ...
                        var message = SimpleMessage.newBuilder().setContent("Hello Infiniworld!").build();
                        return service.streamingRequestAndResponse(Flux.just(message).repeat());
                    })
        ...
        try (var serverSocket = new ServerSocket(port);
            ...
            var rsocket = RSocketFactory.receive()
                    ...
                    .acceptor((setup, reactiveSocket) -> {
                        ...
                        return Mono.just(new RequestHandlingRSocket(serviceServer));
                    })
                    .transport(transport)
                    .start()
                    .subscribe();
